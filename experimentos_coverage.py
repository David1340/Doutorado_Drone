...
p = 30 # proporção de área de interesse em relação à área total
...
for _ in range(quantidade):
   ...
   # print("Coverage Path length (LawMower): ", path_length_LawMower/1000, " km")
    ...
    for i in range(np.size(waypoints,0)-1):
        ...
        path_cost_Hilbert += custo(waypoints[i], waypoints[i+1])
    #print("Coverage Path length Hilbert: ", path_length2/1000, " km")

    data_Hilbert.append([path_length_Hilbert/path_length_LawMower, path_cost_Hilbert/path_cost_LawMower])
    data_Shortcut.append([path_length_ShortCut/path_length_LawMower, path_cost_ShortCut/path_cost_LawMower])
    data_DepthFirst.append([path_length_DeepFirst/path_length_LawMower, path_cost_DeepFirst/path_cost_LawMower])
    data_BreadthFirst.append([path_length_BreadthFirst/path_length_LawMower, path_cost_BreadthFirst/path_cost_LawMower])
...
print("Resultados:")
print("Coverage Path length (LawMower): ", path_length_LawMower/1000, " km")
print(f"Hilbert: {np.mean(data_Hilbert,0)[0]:.2f} +/- {np.std(data_Hilbert,0)[0]:.2f}")
print(f"ShortCut: {np.mean(data_Shortcut,0)[0]:.2f} +/- {np.std(data_Shortcut,0)[0]:.2f}")
print(f"DepthFirst: {np.mean(data_DepthFirst,0)[0]:.2f} +/- {np.std(data_DepthFirst,0)[0]:.2f}")
print(f"BreadthFirst: {np.mean(data_BreadthFirst,0)[0]:.2f} +/- {np.std(data_BreadthFirst,0)[0]:.2f}")

print("Coverage Path cost (LawMower): ", path_cost_LawMower, " s")
print(f"Hilbert: {np.mean(data_Hilbert,0)[1]:.2f} +/- {np.std(data_Hilbert,0)[1]:.2f}")
print(f"ShortCut: {np.mean(data_Shortcut,0)[1]:.2f} +/- {np.std(data_Shortcut,0)[1]:.2f}")
print(f"DepthFirst: {np.mean(data_DepthFirst,0)[1]:.2f} +/- {np.std(data_DepthFirst,0)[1]:.2f}")
print(f"BreadthFirst: {np.mean(data_BreadthFirst,0)[1]:.2f} +/- {np.std(data_BreadthFirst,0)[1]:.2f}")
